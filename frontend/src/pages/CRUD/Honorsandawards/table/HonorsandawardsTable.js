...
import actions from 'actions/honorsandawards/honorsandawardsListActions';
import actionsHonorsandawards from 'actions/honorsandawards/honorsandawardsFormActions';
...
const HonorsandawardsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Title of award', title: 'titleofaward' },
    {
      label: 'Name of awarding organization',
      title: 'nameofawardingorganization',
    },
    {
      label: 'Contacts of awarding organization',
      title: 'contactsofawardingorganization',
    },
    {
      label: 'Brief details of work honored',
      title: 'briefdetailsofworkhonored',
    },
    { label: 'Prize amount', title: 'prizeamount' },
    { label: 'Award winner name', title: 'awardwinnername' },
    { label: 'Award winner designation', title: 'awardwinnerdesignation' },
    { label: 'Award winner department', title: 'awardwinnerdepartment' },
    { label: 'Remarks', title: 'remarks' },
    { label: 'Annex page ref no', title: 'annexpagerefno', number: 'true' },
  ...
  const count = useSelector((store) => store.honorsandawards.list.count);
  const modalOpen = useSelector(
    (store) => store.honorsandawards.list.modalOpen,
  );
  const rows = useSelector((store) => store.honorsandawards.list.rows);
  const idToDelete = useSelector(
    (store) => store.honorsandawards.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsHonorsandawards.doUpdate(id, data));
  };
  ...
  const sendHonorsandawardsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('honorsandawards/bulk-import', data, {
        headers: {
  ...
  const getHonorsandawardsCSV = async () => {
    const response = await axios({
      url: '/honorsandawards?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'honorsandawardsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      ...
      headerName: 'Title of award',
    },

    {
      field: 'nameofawardingorganization',

      flex: 0.6,
      editable: true,

      headerName: 'Name of awarding organization',
    },

    {
      field: 'contactsofawardingorganization',

      flex: 0.6,
      editable: true,

      headerName: 'Contacts of awarding organization',
    },

    {
      field: 'briefdetailsofworkhonored',

      flex: 0.6,
      editable: true,

      headerName: 'Brief details of work honored',
    },
    ...
    {
      field: 'awardwinnername',

      flex: 0.6,
      editable: true,

      headerName: 'Award winner name',
    },

    {
      field: 'awardwinnerdesignation',

      flex: 0.6,
      editable: true,

      headerName: 'Award winner designation',
    },

    {
      field: 'awardwinnerdepartment',

      flex: 0.6,
      editable: true,

      headerName: 'Award winner department',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Honorsandawards')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/honorsandawards/new' className={classes.element}>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendHonorsandawardsCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for honorsandawards
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default HonorsandawardsTable;
