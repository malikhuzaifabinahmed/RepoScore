...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/honorsandawards/autocomplete`, { params });
  return response.data;
}

const HonorsandawardsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.honorsandawards.hasPermissionToCreate,
});

export default connect(select)(HonorsandawardsSelectItem);
