...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/researchlinks/autocomplete`, { params });
  return response.data;
}

const ResearchlinksSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.researchlinks.hasPermissionToCreate,
});

export default connect(select)(ResearchlinksSelectItem);
