...
import actions from 'actions/bicfundinginfo/bicfundinginfoListActions';
import actionsBicfundinginfo from 'actions/bicfundinginfo/bicfundinginfoFormActions';
...
const BicfundinginfoTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'Name of funding agency', title: 'nameoffundingagency' },
    { label: 'National or International', title: 'nationalorinternational' },
    { label: 'Type of funding', title: 'typeoffunding' },
    { label: 'Amount secured', title: 'amountsecured' },
    {
      label: 'Amount utilized distributed',
      title: 'amountutilizeddistributed',
    },
    {
      label: 'In kind support from funding agency (if any)',
      title: 'inkindsupportfromfundingagencyifany',
    },
    {
      label: 'Agreement signed with funding agency (if any) ',
      title: 'agreementsignedwithfundingagencyifany',
    },
    {
      label: 'Recurring or one type support',
      title: 'recurringoronetypesupport',
    },
  ...
  const count = useSelector((store) => store.bicfundinginfo.list.count);
  const modalOpen = useSelector((store) => store.bicfundinginfo.list.modalOpen);
  const rows = useSelector((store) => store.bicfundinginfo.list.rows);
  const idToDelete = useSelector(
    (store) => store.bicfundinginfo.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsBicfundinginfo.doUpdate(id, data));
  };
  ...
  const sendBicfundinginfoCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('bicfundinginfo/bulk-import', data, {
        headers: {
  ...
  const getBicfundinginfoCSV = async () => {
    const response = await axios({
      url: '/bicfundinginfo?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'bicfundinginfoCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'nameoffundingagency',

      flex: 0.6,
      editable: true,

      headerName: 'Name of funding agency',
    },
...

    {
      ...
      headerName: 'Amount secured',
    },

    {
      field: 'amountutilizeddistributed',

      flex: 0.6,
      editable: true,

      headerName: 'Amount utilized distributed',
    },

    {
      field: 'inkindsupportfromfundingagencyifany',

      flex: 0.6,
      editable: true,

      headerName: 'In kind support from funding agency (if any)',
    },

    {
      field: 'agreementsignedwithfundingagencyifany',

      flex: 0.6,
      editable: true,

      headerName: 'Agreement signed with funding agency (if any) ',
    },

    {
      field: 'recurringoronetypesupport',

      flex: 0.6,
      editable: true,

      headerName: 'Recurring or one type support',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Bicfundinginfo')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/bicfundinginfo/new' className={classes.element}>
...

        {
          <div>
            ...
            >
              API documentation for bicfundinginfo
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default BicfundinginfoTable;
