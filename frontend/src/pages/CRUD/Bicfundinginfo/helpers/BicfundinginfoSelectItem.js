...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bicfundinginfo/autocomplete`, { params });
  return response.data;
}

const BicfundinginfoSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bicfundinginfo.hasPermissionToCreate,
});

export default connect(select)(BicfundinginfoSelectItem);
