...
async function selectList(query, limit) {
  ...
  const response = await axios.get(
    `/graduatedstartupfacilitationinfo/autocomplete`,
    { params },
  );
  return response.data;
}

const GraduatedstartupfacilitationinfoSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate:
    state.graduatedstartupfacilitationinfo.hasPermissionToCreate,
});

export default connect(select)(GraduatedstartupfacilitationinfoSelectItem);
