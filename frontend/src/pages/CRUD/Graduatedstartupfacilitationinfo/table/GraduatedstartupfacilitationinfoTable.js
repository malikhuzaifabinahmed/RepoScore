...
import actions from 'actions/graduatedstartupfacilitationinfo/graduatedstartupfacilitationinfoListActions';
import actionsGraduatedstartupfacilitationinfo from 'actions/graduatedstartupfacilitationinfo/graduatedstartupfacilitationinfoFormActions';
...
const GraduatedstartupfacilitationinfoTable = () => {
  ...
  const [filters, setFilters] = useState([
    {
      ...
      title: 'keyareasoffacilitationrequestedbygraduatedstartups',
    },
    {
      label: 'Type of facilitation mentoring trainings provided to graduated',
      title: 'typeoffacilitationmentoringtrainingsprovidedtograduated',
    },
    {
      label: 'No of graduated startups facilitated',
      title: 'noofgraduatedstartupsfacilitated',
    },
    {
      label: 'Total startups incubated since BICs inception',
      title: 'totalstartupsincubatedsincebicsinception',
    },
    {
      label: 'Total startups graduated since BICs inception',
      title: 'totalstartupsgraduatedsincebicsinception',
    },
  ...
  const count = useSelector(
    (store) => store.graduatedstartupfacilitationinfo.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.graduatedstartupfacilitationinfo.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.graduatedstartupfacilitationinfo.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.graduatedstartupfacilitationinfo.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsGraduatedstartupfacilitationinfo.doUpdate(id, data));
  };
  ...
  const sendGraduatedstartupfacilitationinfoCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'graduatedstartupfacilitationinfo/bulk-import',
        data,
        {
          headers: {
  ...
  const getGraduatedstartupfacilitationinfoCSV = async () => {
    const response = await axios({
      url: '/graduatedstartupfacilitationinfo?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'graduatedstartupfacilitationinfoCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'keyareasoffacilitationrequestedbygraduatedstartups',

      flex: 0.6,
      editable: true,

      headerName: 'Key Areas of facilitation requested by graduated startups',
    },

    {
      field: 'typeoffacilitationmentoringtrainingsprovidedtograduated',

      flex: 0.6,
      editable: true,

      headerName:
        'Type of facilitation mentoring trainings provided to graduated',
    },

    {
      field: 'noofgraduatedstartupsfacilitated',

      flex: 0.6,
      editable: true,

      headerName: 'No of graduated startups facilitated',
    },

    {
      field: 'totalstartupsincubatedsincebicsinception',

      flex: 0.6,
      editable: true,

      headerName: 'Total startups incubated since BICs inception',
    },

    {
      field: 'totalstartupsgraduatedsincebicsinception',

      flex: 0.6,
      editable: true,

      headerName: 'Total startups graduated since BICs inception',
    },
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='graduatedstartupfacilitationinfo'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget
        title={humanize('Graduatedstartupfacilitationinfo')}
        disableWidgetMenu
      >
        <Box className={classes.actions}>
          <Link
            to='/admin/graduatedstartupfacilitationinfo/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getGraduatedstartupfacilitationinfoCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendGraduatedstartupfacilitationinfoCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for graduatedstartupfacilitationinfo
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default GraduatedstartupfacilitationinfoTable;
