...
import actions from 'actions/commities/commitiesListActions';
import actionsCommities from 'actions/commities/commitiesFormActions';
...
const CommitiesTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'Designation', title: 'designation' },
    {
      ...
      title: 'parentinstitutionorganization',
    },
    { label: 'Sector field', title: 'sectorfield' },
    { label: 'Role status in committee', title: 'rolestatusincommittee' },
    { label: 'Notified', title: 'notified' },
    { label: 'No. of members', title: 'noofmembers', number: 'true' },
  ...
  const count = useSelector((store) => store.commities.list.count);
  const modalOpen = useSelector((store) => store.commities.list.modalOpen);
  const rows = useSelector((store) => store.commities.list.rows);
  const idToDelete = useSelector((store) => store.commities.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsCommities.doUpdate(id, data));
  };
  ...
  const sendCommitiesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('commities/bulk-import', data, {
        headers: {
  ...
  const getCommitiesCSV = async () => {
    const response = await axios({
      url: '/commities?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'commitiesCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'parentinstitutionorganization',

      flex: 0.6,
      editable: true,

      headerName: 'Parent institution organization',
    },
    ...
    {
      field: 'rolestatusincommittee',

      flex: 0.6,
      editable: true,

      headerName: 'Role status in committee',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Commities')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/commities/new' className={classes.element}>
...

        {
          <div>
            ...
            >
              API documentation for commities
            </LinkMaterial>
          </div>
        }
      </Widget>
