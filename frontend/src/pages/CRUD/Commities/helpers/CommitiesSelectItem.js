...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/commities/autocomplete`, { params });
  return response.data;
}

const CommitiesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.commities.hasPermissionToCreate,
});

export default connect(select)(CommitiesSelectItem);
