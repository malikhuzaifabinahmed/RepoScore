...
import actions from 'actions/humanresource/humanresourceListActions';
import actionsHumanresource from 'actions/humanresource/humanresourceFormActions';
...
const HumanresourceTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'period up to', title: 'periodupto', number: 'true' },
    { label: 'Total experience', title: 'totalexperience', number: 'true' },
    {
      label: 'Nonacademic experience',
      title: 'nonacademicexperience',
      number: 'true',
    },
  ...
  const count = useSelector((store) => store.humanresource.list.count);
  const modalOpen = useSelector((store) => store.humanresource.list.modalOpen);
  const rows = useSelector((store) => store.humanresource.list.rows);
  const idToDelete = useSelector(
    (store) => store.humanresource.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsHumanresource.doUpdate(id, data));
  };
  ...
  const sendHumanresourceCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('humanresource/bulk-import', data, {
        headers: {
  ...
  const getHumanresourceCSV = async () => {
    const response = await axios({
      url: '/humanresource?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'humanresourceCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'nonacademicexperience',

      flex: 0.6,
      editable: true,

      headerName: 'Nonacademic experience',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Humanresource')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/humanresource/new' className={classes.element}>
...

        {
          <div>
            ...
            >
              API documentation for humanresource
            </LinkMaterial>
          </div>
        }
      </Widget>
