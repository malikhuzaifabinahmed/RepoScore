...
import * as dataFormat from 'pages/CRUD/Engagementevents/table/EngagementeventsDataFormatters';

// eslint-disable-next-line
import * as categoriesDataFormat from 'pages/CRUD/Categories/table/CategoriesDataFormatters';

import actions from 'actions/engagementevents/engagementeventsListActions';
import actionsEngagementevents from 'actions/engagementevents/engagementeventsFormActions';
...
const EngagementeventsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Title of event', title: 'titleofevent' },
    {
      label: 'Component of community involved',
      title: 'componentofcommunityinvolved',
    },
    { label: 'Audiance', title: 'audiance' },
    { label: 'Outcome', title: 'outcome' },
    { label: 'Collaboration developed', title: 'collaborationdeveloped' },
    {
      label: 'Name of collaborating partners',
      title: 'nameofcollaboratingpartners',
    },
    { label: 'Name of sponsoring agency', title: 'nameofsponsoringagency' },
    {
      label: 'Will be participated or arranged',
      title: 'willbeparticipatedorarranged',
    },
  ...
  const count = useSelector((store) => store.engagementevents.list.count);
  const modalOpen = useSelector(
    (store) => store.engagementevents.list.modalOpen,
  );
  const rows = useSelector((store) => store.engagementevents.list.rows);
  const idToDelete = useSelector(
    (store) => store.engagementevents.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsEngagementevents.doUpdate(id, data));
  };
  ...
  const sendEngagementeventsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('engagementevents/bulk-import', data, {
        headers: {
  ...
  const getEngagementeventsCSV = async () => {
    const response = await axios({
      url: '/engagementevents?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'engagementeventsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'componentofcommunityinvolved',

      flex: 0.6,
      editable: true,

      headerName: 'Component of community involved',
    },
    ...
    {
      field: 'collaborationdeveloped',

      flex: 0.6,
      editable: true,

      headerName: 'Collaboration developed',
    },
    ...
    {
      field: 'willbeparticipatedorarranged',

      flex: 0.6,
      editable: true,

      headerName: 'Will be participated or arranged',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Engagementevents')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/engagementevents/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getEngagementeventsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendEngagementeventsCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for engagementevents
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default EngagementeventsTable;
