...
import * as dataFormat from 'pages/CRUD/Researchpolicy/table/ResearchpolicyDataFormatters';

// eslint-disable-next-line
import * as categoriesDataFormat from 'pages/CRUD/Categories/table/CategoriesDataFormatters';

import actions from 'actions/researchpolicy/researchpolicyListActions';
import actionsResearchpolicy from 'actions/researchpolicy/researchpolicyFormActions';
...
const ResearchpolicyTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Title of research policy', title: 'titleofresearchpolicy' },
    {
      label: 'Research policy reference number',
      title: 'researchpolicyreferencenumber',
    },
    { label: 'Approved by', title: 'approvedby' },
  ...
  const count = useSelector((store) => store.researchpolicy.list.count);
  const modalOpen = useSelector((store) => store.researchpolicy.list.modalOpen);
  const rows = useSelector((store) => store.researchpolicy.list.rows);
  const idToDelete = useSelector(
    (store) => store.researchpolicy.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsResearchpolicy.doUpdate(id, data));
  };
  ...
  const sendResearchpolicyCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('researchpolicy/bulk-import', data, {
        headers: {
  ...
  const getResearchpolicyCSV = async () => {
    const response = await axios({
      url: '/researchpolicy?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'researchpolicyCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'titleofresearchpolicy',

      flex: 0.6,
      editable: true,

      headerName: 'Title of research policy',
    },

    {
      field: 'researchpolicyreferencenumber',

      flex: 0.6,
      editable: true,

      headerName: 'Research policy reference number',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Researchpolicy')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/researchpolicy/new' className={classes.element}>
...

        {
          <div>
            ...
            >
              API documentation for researchpolicy
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default ResearchpolicyTable;
