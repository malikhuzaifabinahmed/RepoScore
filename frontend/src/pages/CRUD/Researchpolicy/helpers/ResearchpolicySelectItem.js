...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/researchpolicy/autocomplete`, { params });
  return response.data;
}

const ResearchpolicySelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.researchpolicy.hasPermissionToCreate,
});

export default connect(select)(ResearchpolicySelectItem);
