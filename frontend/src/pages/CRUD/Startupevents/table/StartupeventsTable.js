...
import * as dataFormat from 'pages/CRUD/Startupevents/table/StartupeventsDataFormatters';

// eslint-disable-next-line
import * as categoriesDataFormat from 'pages/CRUD/Categories/table/CategoriesDataFormatters';

import actions from 'actions/startupevents/startupeventsListActions';
import actionsStartupevents from 'actions/startupevents/startupeventsFormActions';
...
const StartupeventsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Name of event', title: 'nameofevent' },
    { label: 'Panelist details', title: 'panelistdetails' },
    { label: 'Ideas applied', title: 'ideasapplied' },
    { label: 'Ideas selected', title: 'ideasselected' },
    { label: 'Winners details', title: 'winnersdetails' },
    { label: 'Prize funding amount', title: 'prizefundingamount' },
    {
      label: 'Event funding sources sponsors',
      title: 'eventfundingsourcessponsors',
    },
  ...
  const count = useSelector((store) => store.startupevents.list.count);
  const modalOpen = useSelector((store) => store.startupevents.list.modalOpen);
  const rows = useSelector((store) => store.startupevents.list.rows);
  const idToDelete = useSelector(
    (store) => store.startupevents.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsStartupevents.doUpdate(id, data));
  };
  ...
  const sendStartupeventsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('startupevents/bulk-import', data, {
        headers: {
  ...
  const getStartupeventsCSV = async () => {
    const response = await axios({
      url: '/startupevents?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'startupeventsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'panelistdetails',

      flex: 0.6,
      editable: true,

      headerName: 'Panelist details',
    },
...

    {
      ...
      headerName: 'Ideas selected',
    },

    {
      field: 'winnersdetails',

      flex: 0.6,
      editable: true,

      headerName: 'Winners details',
    },

    {
      field: 'prizefundingamount',

      flex: 0.6,
      editable: true,

      headerName: 'Prize funding amount',
    },

    {
      field: 'eventfundingsourcessponsors',

      flex: 0.6,
      editable: true,

      headerName: 'Event funding sources sponsors',
    },
...

    {
      ...
      headerName: 'Documentaryevidence',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Startupevents')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/startupevents/new' className={classes.element}>
...

        {
          <div>
            ...
            >
              API documentation for startupevents
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default StartupeventsTable;
