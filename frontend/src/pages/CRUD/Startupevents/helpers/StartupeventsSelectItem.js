...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/startupevents/autocomplete`, { params });
  return response.data;
}

const StartupeventsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.startupevents.hasPermissionToCreate,
});

export default connect(select)(StartupeventsSelectItem);
