...
import * as dataFormat from 'pages/CRUD/Universityadvancedstudiesandresearchboard/table/UniversityadvancedstudiesandresearchboardDataFormatters';

// eslint-disable-next-line
import * as categoriesDataFormat from 'pages/CRUD/Categories/table/CategoriesDataFormatters';

import actions from 'actions/universityadvancedstudiesandresearchboard/universityadvancedstudiesandresearchboardListActions';
import actionsUniversityadvancedstudiesandresearchboard from 'actions/universityadvancedstudiesandresearchboard/universityadvancedstudiesandresearchboardFormActions';
...
const UniversityadvancedstudiesandresearchboardTable = () => {
  ...
  const [filters, setFilters] = useState([
    {
      ...
      title: 'nameofmemberdevelopedwith',
    },
  ...
  const count = useSelector(
    (store) => store.universityadvancedstudiesandresearchboard.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.universityadvancedstudiesandresearchboard.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.universityadvancedstudiesandresearchboard.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.universityadvancedstudiesandresearchboard.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(
      actionsUniversityadvancedstudiesandresearchboard.doUpdate(id, data),
    );
  };
  ...
  const sendUniversityadvancedstudiesandresearchboardCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'universityadvancedstudiesandresearchboard/bulk-import',
        data,
        {
          headers: {
  ...
  const getUniversityadvancedstudiesandresearchboardCSV = async () => {
    const response = await axios({
      url: '/universityadvancedstudiesandresearchboard?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'universityadvancedstudiesandresearchboardCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'nameofmemberdevelopedwith',

      flex: 0.6,
      editable: true,

      headerName: 'Name of member developed with',
    },
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='universityadvancedstudiesandresearchboard'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget
        title={humanize('Universityadvancedstudiesandresearchboard')}
        disableWidgetMenu
      >
        <Box className={classes.actions}>
          <Link
            to='/admin/universityadvancedstudiesandresearchboard/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getUniversityadvancedstudiesandresearchboardCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendUniversityadvancedstudiesandresearchboardCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for universityadvancedstudiesandresearchboard
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default UniversityadvancedstudiesandresearchboardTable;
