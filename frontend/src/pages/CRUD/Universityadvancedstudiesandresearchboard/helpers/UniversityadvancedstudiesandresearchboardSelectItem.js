...
async function selectList(query, limit) {
  ...
  const response = await axios.get(
    `/universityadvancedstudiesandresearchboard/autocomplete`,
    { params },
  );
  return response.data;
}

const UniversityadvancedstudiesandresearchboardSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate:
    state.universityadvancedstudiesandresearchboard.hasPermissionToCreate,
});

export default connect(select)(
  UniversityadvancedstudiesandresearchboardSelectItem,
);
