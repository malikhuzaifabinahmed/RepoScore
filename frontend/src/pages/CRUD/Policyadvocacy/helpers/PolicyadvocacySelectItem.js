...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/policyadvocacy/autocomplete`, { params });
  return response.data;
}

const PolicyadvocacySelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.policyadvocacy.hasPermissionToCreate,
});

export default connect(select)(PolicyadvocacySelectItem);
