...
import actions from 'actions/policyadvocacy/policyadvocacyListActions';
import actionsPolicyadvocacy from 'actions/policyadvocacy/policyadvocacyFormActions';
...
const PolicyadvocacyTable = () => {
  ...
  const [filters, setFilters] = useState([
    {
      ...
      title: 'nameofgovernmentbodypresented',
    },
    { label: 'Name of PI', title: 'nameofpi' },
    { label: 'PI designation', title: 'pidesignation' },
    { label: 'PI department', title: 'pidepartment' },
    { label: 'Area advocated', title: 'areaadvocated' },
    { label: 'Description', title: 'description' },
    { label: 'Name coalition partner', title: 'namecoalitionpartner' },
    { label: 'Issue verification', title: 'issueverification' },
    { label: 'Backing research status', title: 'backingresearchstatus' },
    { label: 'Advocacy tools adopted', title: 'advocacytoolsadopted' },
    { label: 'Anex page no', title: 'anexpageno' },
  ...
  const count = useSelector((store) => store.policyadvocacy.list.count);
  const modalOpen = useSelector((store) => store.policyadvocacy.list.modalOpen);
  const rows = useSelector((store) => store.policyadvocacy.list.rows);
  const idToDelete = useSelector(
    (store) => store.policyadvocacy.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsPolicyadvocacy.doUpdate(id, data));
  };
  ...
  const sendPolicyadvocacyCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('policyadvocacy/bulk-import', data, {
        headers: {
  ...
  const getPolicyadvocacyCSV = async () => {
    const response = await axios({
      url: '/policyadvocacy?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'policyadvocacyCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'nameofgovernmentbodypresented',

      flex: 0.6,
      editable: true,

      headerName: 'Name of government body presented',
    },
    ...
    {
      field: 'namecoalitionpartner',

      flex: 0.6,
      editable: true,

      headerName: 'Name coalition partner',
    },

    {
      field: 'issueverification',

      flex: 0.6,
      editable: true,

      headerName: 'Issue verification',
    },

    {
      field: 'backingresearchstatus',

      flex: 0.6,
      editable: true,

      headerName: 'Backing research status',
    },

    {
      field: 'advocacytoolsadopted',

      flex: 0.6,
      editable: true,

      headerName: 'Advocacy tools adopted',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Policyadvocacy')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/policyadvocacy/new' className={classes.element}>
...

        {
          <div>
            ...
            >
              API documentation for policyadvocacy
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default PolicyadvocacyTable;
