...
const PolicyadvocacyForm = (props) => {
  ...
  const formValidations = () => {
    return FormValidations(policyadvocacyFields, record || {});
  };

  const handleSubmit = (values) => {
    const { id, ...data } = PreparedValues(policyadvocacyFields, values || {});
    onSubmit(id, data);
  };
  ...
  const title = () => {
    ...
    return isEditing ? 'Edit Policyadvocacy' : 'Add Policyadvocacy';
  };
        ...
        {(form) => (
          <form onSubmit={form.handleSubmit}>
            <Grid container spacing={3} direction='column'>
              <Grid item>
                <InputFormItem
                  name={'nameofgovernmentbodypresented'}
