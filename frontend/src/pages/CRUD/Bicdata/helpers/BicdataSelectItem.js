...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bicdata/autocomplete`, { params });
  return response.data;
}

const BicdataSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bicdata.hasPermissionToCreate,
});

export default connect(select)(BicdataSelectItem);
