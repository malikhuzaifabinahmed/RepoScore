...
import * as dataFormat from 'pages/CRUD/Startupsincubated/table/StartupsincubatedDataFormatters';

// eslint-disable-next-line
import * as categoriesDataFormat from 'pages/CRUD/Categories/table/CategoriesDataFormatters';

import actions from 'actions/startupsincubated/startupsincubatedListActions';
import actionsStartupsincubated from 'actions/startupsincubated/startupsincubatedFormActions';
...
const StartupsincubatedTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Name of startup', title: 'nameofstartup' },
    { label: 'Brief idea', title: 'briefidea' },
    { label: 'Faculty member name', title: 'facultymembername' },
    { label: 'Faculty member designation', title: 'facultymemberdesignation' },
    { label: 'Faculty member department', title: 'facultymemberdepartment' },
    { label: 'Sector field', title: 'sectorfield' },
    {
      label: 'Seed funding secured (if any)',
      title: 'seedfundingsecuredifany',
    },
    { label: 'Stage', title: 'stage' },
    {
      label: 'Incubatedsinceexpectedgraduation',
      title: 'incubatedsinceexpectedgraduation',
    },
    { label: 'Internship / jobs created', title: 'internshipjobscreated' },
    { label: 'Total faculty startups', title: 'totalfacultystartups' },
  ...
  const count = useSelector((store) => store.startupsincubated.list.count);
  const modalOpen = useSelector(
    (store) => store.startupsincubated.list.modalOpen,
  );
  const rows = useSelector((store) => store.startupsincubated.list.rows);
  const idToDelete = useSelector(
    (store) => store.startupsincubated.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsStartupsincubated.doUpdate(id, data));
  };
  ...
  const sendStartupsincubatedCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('startupsincubated/bulk-import', data, {
        headers: {
  ...
  const getStartupsincubatedCSV = async () => {
    const response = await axios({
      url: '/startupsincubated?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'startupsincubatedCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'facultymemberdesignation',

      flex: 0.6,
      editable: true,

      headerName: 'Faculty member designation',
    },

    {
      field: 'facultymemberdepartment',

      flex: 0.6,
      editable: true,

      headerName: 'Faculty member department',
    },

    {
      ...
      headerName: 'Sector field',
    },

    {
      field: 'seedfundingsecuredifany',

      flex: 0.6,
      editable: true,

      headerName: 'Seed funding secured (if any)',
    },
    ...
    {
      field: 'incubatedsinceexpectedgraduation',

      flex: 0.6,
      editable: true,

      headerName: 'Incubatedsinceexpectedgraduation',
    },

    {
      field: 'internshipjobscreated',

      flex: 0.6,
      editable: true,

      headerName: 'Internship / jobs created',
    },

    {
      field: 'totalfacultystartups',

      flex: 0.6,
      editable: true,

      headerName: 'Total faculty startups',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Startupsincubated')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/startupsincubated/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getStartupsincubatedCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendStartupsincubatedCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for startupsincubated
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default StartupsincubatedTable;
