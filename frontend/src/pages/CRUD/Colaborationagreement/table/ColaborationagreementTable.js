...
import actions from 'actions/colaborationagreement/colaborationagreementListActions';
import actionsColaborationagreement from 'actions/colaborationagreement/colaborationagreementFormActions';
...
const ColaborationagreementTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Type of linkages', title: 'typeoflinkages' },
    { label: 'National or International', title: 'nationalinternational' },
    { label: 'Host country name', title: 'hostcountryname' },
    { label: 'Host country address', title: 'hostcountryaddress' },
    {
      label: 'Key initiatives to be undertaken',
      title: 'keyinitiativestobeundertaken',
    },
  ...
  const count = useSelector((store) => store.colaborationagreement.list.count);
  const modalOpen = useSelector(
    (store) => store.colaborationagreement.list.modalOpen,
  );
  const rows = useSelector((store) => store.colaborationagreement.list.rows);
  const idToDelete = useSelector(
    (store) => store.colaborationagreement.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsColaborationagreement.doUpdate(id, data));
  };
  ...
  const sendColaborationagreementCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'colaborationagreement/bulk-import',
        data,
        {
          headers: {
  ...
  const getColaborationagreementCSV = async () => {
    const response = await axios({
      url: '/colaborationagreement?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'colaborationagreementCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'typeoflinkages',

      flex: 0.6,
      editable: true,

      headerName: 'Type of linkages',
    },
    ...
    {
      field: 'hostcountryname',

      flex: 0.6,
      editable: true,

      headerName: 'Host country name',
    },

    {
      field: 'hostcountryaddress',

      flex: 0.6,
      editable: true,

      headerName: 'Host country address',
    },
    ...
    {
      field: 'keyinitiativestobeundertaken',

      flex: 0.6,
      editable: true,

      headerName: 'Key initiatives to be undertaken',
    },
    ...
    {
      field: 'dateoflinkageestablishment',

      headerName: 'Date of linkage establishment',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Colaborationagreement')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/colaborationagreement/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getColaborationagreementCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendColaborationagreementCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for colaborationagreement
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default ColaborationagreementTable;
