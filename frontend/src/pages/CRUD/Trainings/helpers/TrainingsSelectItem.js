...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/trainings/autocomplete`, { params });
  return response.data;
}

const TrainingsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.trainings.hasPermissionToCreate,
});

export default connect(select)(TrainingsSelectItem);
