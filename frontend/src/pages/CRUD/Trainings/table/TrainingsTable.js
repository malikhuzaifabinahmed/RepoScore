...
import * as dataFormat from 'pages/CRUD/Trainings/table/TrainingsDataFormatters';

// eslint-disable-next-line
import * as categoriesDataFormat from 'pages/CRUD/Categories/table/CategoriesDataFormatters';

import actions from 'actions/trainings/trainingsListActions';
import actionsTrainings from 'actions/trainings/trainingsFormActions';
...
const TrainingsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Title of training', title: 'titleoftraining' },
    { label: 'Number of participants', title: 'numberofparticipants' },
    {
      label: 'Major focus area and outcomes',
      title: 'majorfocusareaandoutcomes',
    },
    { label: 'Audience type', title: 'audiencetype' },
    { label: 'Organizer', title: 'organizer' },
    {
      label: 'Name of ORIC personal attended',
      title: 'nameoforicpersonalattended',
    },
    {
      label: 'Details of ORIC personnel attended',
      title: 'detailsoforicpersonnelattended',
    },
  ...
  const count = useSelector((store) => store.trainings.list.count);
  const modalOpen = useSelector((store) => store.trainings.list.modalOpen);
  const rows = useSelector((store) => store.trainings.list.rows);
  const idToDelete = useSelector((store) => store.trainings.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsTrainings.doUpdate(id, data));
  };
  ...
  const sendTrainingsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('trainings/bulk-import', data, {
        headers: {
  ...
  const getTrainingsCSV = async () => {
    const response = await axios({
      url: '/trainings?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'trainingsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'titleoftraining',

      flex: 0.6,
      editable: true,

      headerName: 'Title of training',
    },
    ...
    {
      field: 'majorfocusareaandoutcomes',

      flex: 0.6,
      editable: true,

      headerName: 'Major focus area and outcomes',
    },
    ...
    {
      field: 'nameoforicpersonalattended',

      flex: 0.6,
      editable: true,

      headerName: 'Name of ORIC personal attended',
    },

    {
      field: 'detailsoforicpersonnelattended',

      flex: 0.6,
      editable: true,

      headerName: 'Details of ORIC personnel attended',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Trainings')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/trainings/new' className={classes.element}>
...

        {
          <div>
            ...
            >
              API documentation for trainings
            </LinkMaterial>
          </div>
        }
      </Widget>
