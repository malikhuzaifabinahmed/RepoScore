...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/workshopeventinfo/autocomplete`, {
    params,
  });
  return response.data;
}

const WorkshopeventinfoSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.workshopeventinfo.hasPermissionToCreate,
});

export default connect(select)(WorkshopeventinfoSelectItem);
