...
import actions from 'actions/workshopeventinfo/workshopeventinfoListActions';
import actionsWorkshopeventinfo from 'actions/workshopeventinfo/workshopeventinfoFormActions';
...
const WorkshopeventinfoTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'Field thematic area', title: 'fieldthematicarea' },
    {
      label: 'Panelist mentor advisor name',
      title: 'panelistmentoradvisorname',
    },
    {
      label: 'Panelist mentor advisor designation',
      title: 'panelistmentoradvisordesignation',
    },
    { label: 'Arranged by', title: 'arrangedby' },
    { label: 'Faculty student', title: 'facultystudent' },
    { label: 'No of participants', title: 'noofparticipants' },
    { label: 'Total no events held', title: 'totalnoeventsheld' },
  ...
  const count = useSelector((store) => store.workshopeventinfo.list.count);
  const modalOpen = useSelector(
    (store) => store.workshopeventinfo.list.modalOpen,
  );
  const rows = useSelector((store) => store.workshopeventinfo.list.rows);
  const idToDelete = useSelector(
    (store) => store.workshopeventinfo.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsWorkshopeventinfo.doUpdate(id, data));
  };
  ...
  const sendWorkshopeventinfoCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('workshopeventinfo/bulk-import', data, {
        headers: {
  ...
  const getWorkshopeventinfoCSV = async () => {
    const response = await axios({
      url: '/workshopeventinfo?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'workshopeventinfoCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'fieldthematicarea',

      flex: 0.6,
      editable: true,

      headerName: 'Field thematic area',
    },

    {
      field: 'panelistmentoradvisorname',

      flex: 0.6,
      editable: true,

      headerName: 'Panelist mentor advisor name',
    },

    {
      field: 'panelistmentoradvisordesignation',

      flex: 0.6,
      editable: true,

      headerName: 'Panelist mentor advisor designation',
    },
    ...
    {
      field: 'facultystudent',

      flex: 0.6,
      editable: true,

      headerName: 'Faculty student',
    },
    ...
    {
      field: 'totalnoeventsheld',

      flex: 0.6,
      editable: true,

      headerName: 'Total no events held',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Workshopeventinfo')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/workshopeventinfo/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getWorkshopeventinfoCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendWorkshopeventinfoCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for workshopeventinfo
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default WorkshopeventinfoTable;
