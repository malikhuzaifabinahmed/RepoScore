...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/oricdata/autocomplete`, { params });
  return response.data;
}

const OricdataSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.oricdata.hasPermissionToCreate,
});

export default connect(select)(OricdataSelectItem);
