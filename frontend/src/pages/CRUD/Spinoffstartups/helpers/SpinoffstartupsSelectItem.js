...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/spinoffstartups/autocomplete`, { params });
  return response.data;
}

const SpinoffstartupsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.spinoffstartups.hasPermissionToCreate,
});

export default connect(select)(SpinoffstartupsSelectItem);
