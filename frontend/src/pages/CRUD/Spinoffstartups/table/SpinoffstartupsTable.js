...
import actions from 'actions/spinoffstartups/spinoffstartupsListActions';
import actionsSpinoffstartups from 'actions/spinoffstartups/spinoffstartupsFormActions';
...
const SpinoffstartupsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Name of startup', title: 'nameofstartup' },
    {
      ...
      title: 'briefideabackingresearchsectorfield',
    },
    {
      label: 'faculty member name designation department',
      title: 'facultymembernamedesignationdepartment',
    },
    { label: 'IP status', title: 'ipstatus' },
    { label: 'Name of spinoff', title: 'nameofspinoff' },
    { label: 'Stage', title: 'stage' },
    {
      label: 'license agreement signed (if any)',
      title: 'licenseagreementsignedifany',
    },
    { label: 'Funding sources', title: 'fundingsources' },
    {
      label: 'total Faculty startups',
      title: 'totalfacultystartups',
      number: 'true',
    },
  ...
  const count = useSelector((store) => store.spinoffstartups.list.count);
  const modalOpen = useSelector(
    (store) => store.spinoffstartups.list.modalOpen,
  );
  const rows = useSelector((store) => store.spinoffstartups.list.rows);
  const idToDelete = useSelector(
    (store) => store.spinoffstartups.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsSpinoffstartups.doUpdate(id, data));
  };
  ...
  const sendSpinoffstartupsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('spinoffstartups/bulk-import', data, {
        headers: {
  ...
  const getSpinoffstartupsCSV = async () => {
    const response = await axios({
      url: '/spinoffstartups?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'spinoffstartupsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'briefideabackingresearchsectorfield',

      flex: 0.6,
      editable: true,

      headerName: 'Brief idea backing research sector field',
    },
    ...
    {
      field: 'licenseagreementsignedifany',

      flex: 0.6,
      editable: true,

      headerName: 'license agreement signed (if any)',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Spinoffstartups')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/spinoffstartups/new' className={classes.element}>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendSpinoffstartupsCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for spinoffstartups
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default SpinoffstartupsTable;
