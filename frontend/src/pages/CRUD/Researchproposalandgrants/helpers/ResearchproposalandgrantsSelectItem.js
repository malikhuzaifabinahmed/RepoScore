...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/researchproposalandgrants/autocomplete`, {
    params,
  });
  return response.data;
}

const ResearchproposalandgrantsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.researchproposalandgrants.hasPermissionToCreate,
});

export default connect(select)(ResearchproposalandgrantsSelectItem);
