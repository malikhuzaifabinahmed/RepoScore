...
import actions from 'actions/trainingcourseinfo/trainingcourseinfoListActions';
import actionsTrainingcourseinfo from 'actions/trainingcourseinfo/trainingcourseinfoFormActions';
...
const TrainingcourseinfoTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Name of course', title: 'nameofcourse' },
    { label: 'Type of course', title: 'typeofcourse' },
    { label: 'Level offered', title: 'leveloffered' },
    { label: 'Major areas covered', title: 'majorareascovered' },
    { label: 'Key learning outcomes', title: 'keylearningoutcomes' },
    {
      label: 'No of startups completed the course',
      title: 'noofstartupscompletedthecourse',
    },
    { label: 'Course development', title: 'coursedevelopment' },
    {
      label: 'If outsourced name of course offered',
      title: 'ifoutsourcednameofcourseoffered',
    },
  ...
  const count = useSelector((store) => store.trainingcourseinfo.list.count);
  const modalOpen = useSelector(
    (store) => store.trainingcourseinfo.list.modalOpen,
  );
  const rows = useSelector((store) => store.trainingcourseinfo.list.rows);
  const idToDelete = useSelector(
    (store) => store.trainingcourseinfo.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsTrainingcourseinfo.doUpdate(id, data));
  };
  ...
  const sendTrainingcourseinfoCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('trainingcourseinfo/bulk-import', data, {
        headers: {
  ...
  const getTrainingcourseinfoCSV = async () => {
    const response = await axios({
      url: '/trainingcourseinfo?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'trainingcourseinfoCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      headerName: 'Level offered',
    },

    {
      field: 'majorareascovered',

      flex: 0.6,
      editable: true,

      headerName: 'Major areas covered',
    },

    {
      field: 'keylearningoutcomes',

      flex: 0.6,
      editable: true,

      headerName: 'Key learning outcomes',
    },

    {
      field: 'noofstartupscompletedthecourse',

      flex: 0.6,
      editable: true,

      headerName: 'No of startups completed the course',
    },

    {
      field: 'coursedevelopment',

      flex: 0.6,
      editable: true,

      headerName: 'Course development',
    },

    {
      field: 'ifoutsourcednameofcourseoffered',

      flex: 0.6,
      editable: true,

      headerName: 'If outsourced name of course offered',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Trainingcourseinfo')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/trainingcourseinfo/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getTrainingcourseinfoCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendTrainingcourseinfoCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for trainingcourseinfo
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default TrainingcourseinfoTable;
