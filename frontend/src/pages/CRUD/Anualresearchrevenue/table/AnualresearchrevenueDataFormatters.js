...
function categoriesManyListFormatter(val) {
  if (!val || !val.length) return [];
  return val.map((item) => item.id);
}
function categoriesOneListFormatter(val) {
  if (!val) return '';
  return val.id;
}
function categoriesManyListFormatterEdit(val) {
  if (!val || !val.length) return [];
  return val.map((item) => {
    return { id: item.id, label: item.id };
  });
}
function categoriesOneListFormatterEdit(val) {
  if (!val) return '';
