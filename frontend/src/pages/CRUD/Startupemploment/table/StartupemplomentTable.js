...
import actions from 'actions/startupemploment/startupemplomentListActions';
import actionsStartupemploment from 'actions/startupemploment/startupemplomentFormActions';
...
const StartupemplomentTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'Startup faculty member name', title: 'startupfacultymembername' },
    { label: 'Designation department', title: 'designationdepartment' },
    {
      label: 'Startup status incubated graduated',
      title: 'startupstatusincubatedgraduated',
    },
    { label: 'Employee name', title: 'employeename' },
    { label: 'Employment type', title: 'employmenttype' },
    { label: 'Salary stipend honorarium', title: 'salarystipendhonorarium' },
    { label: 'Employee since', title: 'employeesince' },
  ...
  const count = useSelector((store) => store.startupemploment.list.count);
  const modalOpen = useSelector(
    (store) => store.startupemploment.list.modalOpen,
  );
  const rows = useSelector((store) => store.startupemploment.list.rows);
  const idToDelete = useSelector(
    (store) => store.startupemploment.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsStartupemploment.doUpdate(id, data));
  };
  ...
  const sendStartupemplomentCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('startupemploment/bulk-import', data, {
        headers: {
  ...
  const getStartupemplomentCSV = async () => {
    const response = await axios({
      url: '/startupemploment?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'startupemplomentCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'startupfacultymembername',

      flex: 0.6,
      editable: true,

      headerName: 'Startup faculty member name',
    },
    ...
    {
      field: 'startupstatusincubatedgraduated',

      flex: 0.6,
      editable: true,

      headerName: 'Startup status incubated graduated',
    },
    ...
    {
      field: 'salarystipendhonorarium',

      flex: 0.6,
      editable: true,

      headerName: 'Salary stipend honorarium',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Startupemploment')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/startupemploment/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getStartupemplomentCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendStartupemplomentCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for startupemploment
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default StartupemplomentTable;
