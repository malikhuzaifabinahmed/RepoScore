...
import actions from 'actions/partnershipinfo/partnershipinfoListActions';
import actionsPartnershipinfo from 'actions/partnershipinfo/partnershipinfoFormActions';
...
const PartnershipinfoTable = () => {
  ...
  const [filters, setFilters] = useState([
    {
      ...
      title: 'nameofpartneringorganization',
    },
    { label: 'National or International', title: 'nationalinternational' },
    { label: 'Field area of expertise', title: 'fieldareaofexpertise' },
    { label: 'Type of link', title: 'typeoflink' },
    {
      label: 'Major areas of cooperation modalities',
      title: 'majorareasofcooperationmodalities',
    },
    { label: 'Key outcomes from links', title: 'keyoutcomesfromlinks' },
  ...
  const count = useSelector((store) => store.partnershipinfo.list.count);
  const modalOpen = useSelector(
    (store) => store.partnershipinfo.list.modalOpen,
  );
  const rows = useSelector((store) => store.partnershipinfo.list.rows);
  const idToDelete = useSelector(
    (store) => store.partnershipinfo.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsPartnershipinfo.doUpdate(id, data));
  };
  ...
  const sendPartnershipinfoCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('partnershipinfo/bulk-import', data, {
        headers: {
  ...
  const getPartnershipinfoCSV = async () => {
    const response = await axios({
      url: '/partnershipinfo?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'partnershipinfoCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'nameofpartneringorganization',

      flex: 0.6,
      editable: true,

      headerName: 'Name of partnering organization',
    },
    ...
    {
      field: 'fieldareaofexpertise',

      flex: 0.6,
      editable: true,

      headerName: 'Field area of expertise',
    },
    ...
    {
      field: 'majorareasofcooperationmodalities',

      flex: 0.6,
      editable: true,

      headerName: 'Major areas of cooperation modalities',
    },

    {
      field: 'keyoutcomesfromlinks',

      flex: 0.6,
      editable: true,

      headerName: 'Key outcomes from links',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Partnershipinfo')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/partnershipinfo/new' className={classes.element}>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendPartnershipinfoCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for partnershipinfo
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default PartnershipinfoTable;
