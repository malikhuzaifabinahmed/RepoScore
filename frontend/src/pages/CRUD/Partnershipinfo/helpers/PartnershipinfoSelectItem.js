...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/partnershipinfo/autocomplete`, { params });
  return response.data;
}

const PartnershipinfoSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.partnershipinfo.hasPermissionToCreate,
});

export default connect(select)(PartnershipinfoSelectItem);
