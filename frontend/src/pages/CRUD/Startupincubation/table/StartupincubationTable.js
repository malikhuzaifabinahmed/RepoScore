...
import actions from 'actions/startupincubation/startupincubationListActions';
import actionsStartupincubation from 'actions/startupincubation/startupincubationFormActions';
...
const StartupincubationTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'Incubated since', title: 'incubatedsince' },
    { label: 'Expected Graduation', title: 'expectedgraduation' },
    {
      label: 'Internship jobs provided via startup',
      title: 'internshipjobsprovidedviastartup',
    },
    { label: 'Total slots', title: 'totalslots' },
    { label: 'Occupied slots', title: 'occupiedslots' },
  ...
  const count = useSelector((store) => store.startupincubation.list.count);
  const modalOpen = useSelector(
    (store) => store.startupincubation.list.modalOpen,
  );
  const rows = useSelector((store) => store.startupincubation.list.rows);
  const idToDelete = useSelector(
    (store) => store.startupincubation.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsStartupincubation.doUpdate(id, data));
  };
  ...
  const sendStartupincubationCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('startupincubation/bulk-import', data, {
        headers: {
  ...
  const getStartupincubationCSV = async () => {
    const response = await axios({
      url: '/startupincubation?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'startupincubationCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'incubatedsince',

      flex: 0.6,
      editable: true,

      headerName: 'Incubated since',
    },
    ...
    {
      field: 'internshipjobsprovidedviastartup',

      flex: 0.6,
      editable: true,

      headerName: 'Internship jobs provided via startup',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Startupincubation')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/startupincubation/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getStartupincubationCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendStartupincubationCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for startupincubation
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default StartupincubationTable;
