...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/startuprevenue/autocomplete`, { params });
  return response.data;
}

const StartuprevenueSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.startuprevenue.hasPermissionToCreate,
});

export default connect(select)(StartuprevenueSelectItem);
