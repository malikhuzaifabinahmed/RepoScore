...
import actions from 'actions/startuprevenue/startuprevenueListActions';
import actionsStartuprevenue from 'actions/startuprevenue/startuprevenueFormActions';
...
const StartuprevenueTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Name of startup', title: 'nameofstartup' },
    {
      ...
      title: 'facultymembernamedesignationdepartment',
    },
    { label: 'Current status', title: 'currentstatus' },
    {
      label: 'Incubated since and expected graduation',
      title: 'incubatedsinceandexpectedgraduation',
    },
    { label: 'Revenue generated', title: 'revenuegenerated' },
    { label: 'Total months in incubation', title: 'totalmonthsinincubation' },
    { label: 'Average revenue', title: 'averagerevenue' },
    {
      label: 'Sharing mechanism between BIC, HEC and startup',
      title: 'sharingmechanismbetweenbicheiandstartup',
    },
  ...
  const count = useSelector((store) => store.startuprevenue.list.count);
  const modalOpen = useSelector((store) => store.startuprevenue.list.modalOpen);
  const rows = useSelector((store) => store.startuprevenue.list.rows);
  const idToDelete = useSelector(
    (store) => store.startuprevenue.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsStartuprevenue.doUpdate(id, data));
  };
  ...
  const sendStartuprevenueCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('startuprevenue/bulk-import', data, {
        headers: {
  ...
  const getStartuprevenueCSV = async () => {
    const response = await axios({
      url: '/startuprevenue?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'startuprevenueCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'facultymembernamedesignationdepartment',

      flex: 0.6,
      editable: true,

      headerName: 'Faculty member name designation department',
    },
    ...
    {
      field: 'incubatedsinceandexpectedgraduation',

      flex: 0.6,
      editable: true,

      headerName: 'Incubated since and expected graduation',
    },
    ...
    {
      field: 'totalmonthsinincubation',

      flex: 0.6,
      editable: true,

      headerName: 'Total months in incubation',
    },

    {
      ...
      headerName: 'Average revenue',
    },

    {
      field: 'sharingmechanismbetweenbicheiandstartup',

      flex: 0.6,
      editable: true,

      headerName: 'Sharing mechanism between BIC, HEC and startup',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Startuprevenue')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/startuprevenue/new' className={classes.element}>
...

        {
          <div>
            ...
            >
              API documentation for startuprevenue
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default StartuprevenueTable;
