...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/mentorshipinfo/autocomplete`, { params });
  return response.data;
}

const MentorshipinfoSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.mentorshipinfo.hasPermissionToCreate,
});

export default connect(select)(MentorshipinfoSelectItem);
