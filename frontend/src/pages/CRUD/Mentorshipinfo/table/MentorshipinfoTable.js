...
import actions from 'actions/mentorshipinfo/mentorshipinfoListActions';
import actionsMentorshipinfo from 'actions/mentorshipinfo/mentorshipinfoFormActions';
...
const MentorshipinfoTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Name of mentor', title: 'nameofmentor' },
    { label: 'Field areas of expertise', title: 'fieldareasofexpertise' },
    { label: 'National or international', title: 'nationalinternational' },
    {
      label: 'Mentoring sessions held at BIC during the year',
      title: 'mentoringsessionsheldatbicduringtheyear',
    },
    {
      label: 'No of startups facilitated through sessions',
      title: 'noofstartupsfacilitatedthroughsessions',
    },
  ...
  const count = useSelector((store) => store.mentorshipinfo.list.count);
  const modalOpen = useSelector((store) => store.mentorshipinfo.list.modalOpen);
  const rows = useSelector((store) => store.mentorshipinfo.list.rows);
  const idToDelete = useSelector(
    (store) => store.mentorshipinfo.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsMentorshipinfo.doUpdate(id, data));
  };
  ...
  const sendMentorshipinfoCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('mentorshipinfo/bulk-import', data, {
        headers: {
  ...
  const getMentorshipinfoCSV = async () => {
    const response = await axios({
      url: '/mentorshipinfo?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'mentorshipinfoCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      ...
      headerName: 'Name of mentor',
    },

    {
      field: 'fieldareasofexpertise',

      flex: 0.6,
      editable: true,

      headerName: 'Field areas of expertise',
    },
    ...
    {
      field: 'mentoringsessionsheldatbicduringtheyear',

      flex: 0.6,
      editable: true,

      headerName: 'Mentoring sessions held at BIC during the year',
    },

    {
      field: 'noofstartupsfacilitatedthroughsessions',

      flex: 0.6,
      editable: true,

      headerName: 'No of startups facilitated through sessions',
    },

    {
      field: 'ifcorporatebodymousigningdateifany',

      headerName: 'if corporate body mou signing date (ifany)',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Mentorshipinfo')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/mentorshipinfo/new' className={classes.element}>
...

        {
          <div>
            ...
            >
              API documentation for mentorshipinfo
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default MentorshipinfoTable;
