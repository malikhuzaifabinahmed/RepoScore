...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/university/autocomplete`, { params });
  return response.data;
}

const UniversitySelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.university.hasPermissionToCreate,
});

export default connect(select)(UniversitySelectItem);
