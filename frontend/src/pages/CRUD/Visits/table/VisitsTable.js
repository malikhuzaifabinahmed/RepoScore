...
import * as dataFormat from 'pages/CRUD/Visits/table/VisitsDataFormatters';

// eslint-disable-next-line
import * as categoriesDataFormat from 'pages/CRUD/Categories/table/CategoriesDataFormatters';

import actions from 'actions/visits/visitsListActions';
import actionsVisits from 'actions/visits/visitsFormActions';
...
const VisitsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Name of visits', title: 'nameofvisits' },
    { label: 'Date of visit', title: 'dateofvisit' },
    { label: 'Agenda of visit', title: 'agendaofvisit' },
  ...
  const count = useSelector((store) => store.visits.list.count);
  const modalOpen = useSelector((store) => store.visits.list.modalOpen);
  const rows = useSelector((store) => store.visits.list.rows);
  const idToDelete = useSelector((store) => store.visits.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsVisits.doUpdate(id, data));
  };
  ...
  return (
    <div>
      <Widget title={humanize('Visits')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/visits/new' className={classes.element}>
