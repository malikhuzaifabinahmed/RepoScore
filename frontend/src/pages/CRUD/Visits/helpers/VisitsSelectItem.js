...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/visits/autocomplete`, { params });
  return response.data;
}

const VisitsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.visits.hasPermissionToCreate,
});

export default connect(select)(VisitsSelectItem);
