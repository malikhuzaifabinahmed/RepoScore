...
import * as dataFormat from 'pages/CRUD/Bicsupportinfo/table/BicsupportinfoDataFormatters';

// eslint-disable-next-line
import * as categoriesDataFormat from 'pages/CRUD/Categories/table/CategoriesDataFormatters';

import actions from 'actions/bicsupportinfo/bicsupportinfoListActions';
import actionsBicsupportinfo from 'actions/bicsupportinfo/bicsupportinfoFormActions';
...
const BicsupportinfoTable = () => {
  ...
  const [filters, setFilters] = useState([
    {
      ...
      title: 'levelofsupportfinancialinkind',
    },
    {
      label: 'Area facilitated through support',
      title: 'areafacilitatedthroughsupport',
    },
    {
      label: 'Financial in kind support extended for activity',
      title: 'financialinkindsupportextendedforactivity',
    },
  ...
  const count = useSelector((store) => store.bicsupportinfo.list.count);
  const modalOpen = useSelector((store) => store.bicsupportinfo.list.modalOpen);
  const rows = useSelector((store) => store.bicsupportinfo.list.rows);
  const idToDelete = useSelector(
    (store) => store.bicsupportinfo.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsBicsupportinfo.doUpdate(id, data));
  };
  ...
  const sendBicsupportinfoCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('bicsupportinfo/bulk-import', data, {
        headers: {
  ...
  const getBicsupportinfoCSV = async () => {
    const response = await axios({
      url: '/bicsupportinfo?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'bicsupportinfoCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'levelofsupportfinancialinkind',

      flex: 0.6,
      editable: true,

      headerName: 'Level of support financial in kind',
    },

    {
      field: 'areafacilitatedthroughsupport',

      flex: 0.6,
      editable: true,

      headerName: 'Area facilitated through support',
    },

    {
      field: 'financialinkindsupportextendedforactivity',

      flex: 0.6,
      editable: true,

      headerName: 'Financial in kind support extended for activity',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Bicsupportinfo')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/bicsupportinfo/new' className={classes.element}>
...

        {
          <div>
            ...
            >
              API documentation for bicsupportinfo
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default BicsupportinfoTable;
