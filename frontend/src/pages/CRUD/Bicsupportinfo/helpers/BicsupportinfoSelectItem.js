...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bicsupportinfo/autocomplete`, { params });
  return response.data;
}

const BicsupportinfoSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bicsupportinfo.hasPermissionToCreate,
});

export default connect(select)(BicsupportinfoSelectItem);
