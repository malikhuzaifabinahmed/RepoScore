...
import actions from 'actions/startupsappliedforpitching/startupsappliedforpitchingListActions';
import actionsStartupsappliedforpitching from 'actions/startupsappliedforpitching/startupsappliedforpitchingFormActions';
...
const StartupsappliedforpitchingTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'Entrepreneur', title: 'entrepreneur' },
    { label: 'Field area', title: 'fieldarea' },
    { label: 'Educational background', title: 'educationalbackground' },
    { label: 'Team members', title: 'teammembers' },
    { label: 'Current status', title: 'currentstatus' },
    {
      label: 'Result remarks from competition',
      title: 'resultremarksfromcompetition',
    },
    {
      label: 'Availability of funding seed money',
      title: 'availabilityoffundingseedmoney',
    },
    { label: 'Date held', title: 'dateheld' },
    { label: 'No of ideas applied', title: 'noofideasapplied' },
  ...
  const count = useSelector(
    (store) => store.startupsappliedforpitching.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.startupsappliedforpitching.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.startupsappliedforpitching.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.startupsappliedforpitching.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsStartupsappliedforpitching.doUpdate(id, data));
  };
  ...
  const sendStartupsappliedforpitchingCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'startupsappliedforpitching/bulk-import',
        data,
        {
          headers: {
  ...
  const getStartupsappliedforpitchingCSV = async () => {
    const response = await axios({
      url: '/startupsappliedforpitching?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'startupsappliedforpitchingCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'resultremarksfromcompetition',

      flex: 0.6,
      editable: true,

      headerName: 'Result remarks from competition',
    },

    {
      field: 'availabilityoffundingseedmoney',

      flex: 0.6,
      editable: true,

      headerName: 'Availability of funding seed money',
    },

    {
      ...
      headerName: 'Date held',
    },

    {
      field: 'noofideasapplied',

      flex: 0.6,
      editable: true,

      headerName: 'No of ideas applied',
    },
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='startupsappliedforpitching'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Startupsappliedforpitching')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/startupsappliedforpitching/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getStartupsappliedforpitchingCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendStartupsappliedforpitchingCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for startupsappliedforpitching
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default StartupsappliedforpitchingTable;
