...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bicserviceofferinginfo/autocomplete`, {
    params,
  });
  return response.data;
}

const BicserviceofferinginfoSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bicserviceofferinginfo.hasPermissionToCreate,
});

export default connect(select)(BicserviceofferinginfoSelectItem);
