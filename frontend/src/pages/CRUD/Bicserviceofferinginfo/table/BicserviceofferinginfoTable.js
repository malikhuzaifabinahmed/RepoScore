...
import actions from 'actions/bicserviceofferinginfo/bicserviceofferinginfoListActions';
import actionsBicserviceofferinginfo from 'actions/bicserviceofferinginfo/bicserviceofferinginfoFormActions';
...
const BicserviceofferinginfoTable = () => {
  ...
  const [filters, setFilters] = useState([
    {
      ...
      title: 'natureofserviceofferedsessionheld',
    },
    { label: 'Name of service provider', title: 'nameofserviceprovider' },
    { label: 'Background and expertise', title: 'backgroundandexpertise' },
    { label: 'local or International', title: 'localinternational' },
    { label: 'No. of startups facilitated', title: 'noofstartupsfacilitated' },
    { label: 'total no. sessions held', title: 'totalnosessionsheld' },
  ...
  const count = useSelector((store) => store.bicserviceofferinginfo.list.count);
  const modalOpen = useSelector(
    (store) => store.bicserviceofferinginfo.list.modalOpen,
  );
  const rows = useSelector((store) => store.bicserviceofferinginfo.list.rows);
  const idToDelete = useSelector(
    (store) => store.bicserviceofferinginfo.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsBicserviceofferinginfo.doUpdate(id, data));
  };
  ...
  const sendBicserviceofferinginfoCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'bicserviceofferinginfo/bulk-import',
        data,
        {
          headers: {
  ...
  const getBicserviceofferinginfoCSV = async () => {
    const response = await axios({
      url: '/bicserviceofferinginfo?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'bicserviceofferinginfoCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'natureofserviceofferedsessionheld',

      flex: 0.6,
      editable: true,

      headerName: 'Nature of service offered session held',
    },

    {
      field: 'nameofserviceprovider',
    ...
    {
      field: 'backgroundandexpertise',

      flex: 0.6,
      editable: true,

      headerName: 'Background and expertise',
    },

    {
      field: 'localinternational',

      flex: 0.6,
      editable: true,

      headerName: 'local or International',
    },

    {
      field: 'noofstartupsfacilitated',

      flex: 0.6,
      editable: true,

      headerName: 'No. of startups facilitated',
    },

    {
      field: 'totalnosessionsheld',

      flex: 0.6,
      editable: true,

      headerName: 'total no. sessions held',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Bicserviceofferinginfo')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/bicserviceofferinginfo/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getBicserviceofferinginfoCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendBicserviceofferinginfoCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for bicserviceofferinginfo
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default BicserviceofferinginfoTable;
