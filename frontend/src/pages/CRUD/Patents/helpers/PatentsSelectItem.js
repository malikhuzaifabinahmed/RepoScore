...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/patents/autocomplete`, { params });
  return response.data;
}

const PatentsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.patents.hasPermissionToCreate,
});

export default connect(select)(PatentsSelectItem);
