...
import actions from 'actions/bichumanresource/bichumanresourceListActions';
import actionsBichumanresource from 'actions/bichumanresource/bichumanresourceFormActions';
...
const BichumanresourceTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'Full time part time', title: 'fulltimeparttime' },
    { label: 'Name', title: 'name' },
    { label: 'Office phone number', title: 'officephonenumber' },
    { label: 'Mobile number', title: 'mobilenumber' },
    { label: 'Email id', title: 'emailid' },
    { label: 'Qualification level', title: 'qualificationlevel' },
    { label: 'Qualification title', title: 'qualificationtitle' },
    { label: 'Field of study', title: 'fieldofstudy' },
    { label: 'CNIC', title: 'cnic' },
    { label: 'Period up to', title: 'periodupto' },
    { label: 'Total experience', title: 'totalexperience' },
    { label: 'Non academia experience', title: 'nonacademiaexperience' },
    {
      label: 'Notification office order joining report and cv',
      title: 'notificationofficeorderjoiningreportandcv',
    },
  ...
  const count = useSelector((store) => store.bichumanresource.list.count);
  const modalOpen = useSelector(
    (store) => store.bichumanresource.list.modalOpen,
  );
  const rows = useSelector((store) => store.bichumanresource.list.rows);
  const idToDelete = useSelector(
    (store) => store.bichumanresource.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsBichumanresource.doUpdate(id, data));
  };
  ...
  const sendBichumanresourceCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('bichumanresource/bulk-import', data, {
        headers: {
  ...
  const getBichumanresourceCSV = async () => {
    const response = await axios({
      url: '/bichumanresource?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'bichumanresourceCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'fulltimeparttime',

      flex: 0.6,
      editable: true,

      headerName: 'Full time part time',
    },
...

    {
      ...
      headerName: 'Office phone number',
    },
...

    {
      ...
      headerName: 'Qualification title',
    },
    ...
    {
      field: 'nonacademiaexperience',

      flex: 0.6,
      editable: true,

      headerName: 'Non academia experience',
    },

    {
      field: 'notificationofficeorderjoiningreportandcv',

      flex: 0.6,
      editable: true,

      headerName: 'Notification office order joining report and cv',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Bichumanresource')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/bichumanresource/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getBichumanresourceCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendBichumanresourceCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for bichumanresource
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default BichumanresourceTable;
