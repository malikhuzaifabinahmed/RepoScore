...
import actions from 'actions/startupmentoring/startupmentoringListActions';
import actionsStartupmentoring from 'actions/startupmentoring/startupmentoringFormActions';
...
const StartupmentoringTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'Field area', title: 'fieldarea' },
    {
      ...
      title: 'noofmentoringlecturesprovided',
    },
    { label: 'No of startups facilitated', title: 'noofstartupsfacilitated' },
    {
      label: 'Cost per mentoring hour charged (if any)',
      title: 'costpermentoringhourchargedifany',
    },
    { label: 'Total mentoring sessions', title: 'totalmentoringsessions' },
  ...
  const count = useSelector((store) => store.startupmentoring.list.count);
  const modalOpen = useSelector(
    (store) => store.startupmentoring.list.modalOpen,
  );
  const rows = useSelector((store) => store.startupmentoring.list.rows);
  const idToDelete = useSelector(
    (store) => store.startupmentoring.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsStartupmentoring.doUpdate(id, data));
  };
  ...
  const sendStartupmentoringCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('startupmentoring/bulk-import', data, {
        headers: {
  ...
  const getStartupmentoringCSV = async () => {
    const response = await axios({
      url: '/startupmentoring?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'startupmentoringCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'noofmentoringlecturesprovided',

      flex: 0.6,
      editable: true,

      headerName: 'No. of mentoring lectures provided',
    },
    ...
    {
      field: 'costpermentoringhourchargedifany',

      flex: 0.6,
      editable: true,

      headerName: 'Cost per mentoring hour charged (if any)',
    },

    {
      field: 'totalmentoringsessions',

      flex: 0.6,
      editable: true,

      headerName: 'Total mentoring sessions',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Startupmentoring')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/startupmentoring/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getStartupmentoringCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendStartupmentoringCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for startupmentoring
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default StartupmentoringTable;
