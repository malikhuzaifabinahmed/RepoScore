...
import actions from 'actions/coursedetailsinfo/coursedetailsinfoListActions';
import actionsCoursedetailsinfo from 'actions/coursedetailsinfo/coursedetailsinfoFormActions';
...
const CoursedetailsinfoTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'Title of course', title: 'titleofcourse' },
    { label: 'Field', title: 'field' },
    { label: 'Level', title: 'level' },
    { label: 'Credit hours.', title: 'credithours' },
    { label: 'Total modules', title: 'totalmodules' },
    { label: 'Optional compulsory', title: 'optionalcompulsory' },
    {
      label: 'Departments schools offering the course',
      title: 'departmentsschoolsofferingthecourse',
    },
    { label: 'Learning outcomes', title: 'learningoutcomes' },
    { label: 'Total no. of courses offered', title: 'totalnoofcoursesoffered' },
  ...
  const count = useSelector((store) => store.coursedetailsinfo.list.count);
  const modalOpen = useSelector(
    (store) => store.coursedetailsinfo.list.modalOpen,
  );
  const rows = useSelector((store) => store.coursedetailsinfo.list.rows);
  const idToDelete = useSelector(
    (store) => store.coursedetailsinfo.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsCoursedetailsinfo.doUpdate(id, data));
  };
  ...
  const sendCoursedetailsinfoCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('coursedetailsinfo/bulk-import', data, {
        headers: {
  ...
  const getCoursedetailsinfoCSV = async () => {
    const response = await axios({
      url: '/coursedetailsinfo?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'coursedetailsinfoCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'optionalcompulsory',

      flex: 0.6,
      editable: true,

      headerName: 'Optional compulsory',
    },

    {
      field: 'departmentsschoolsofferingthecourse',

      flex: 0.6,
      editable: true,

      headerName: 'Departments schools offering the course',
    },
    ...
    {
      field: 'totalnoofcoursesoffered',

      flex: 0.6,
      editable: true,

      headerName: 'Total no. of courses offered',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Coursedetailsinfo')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/coursedetailsinfo/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getCoursedetailsinfoCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendCoursedetailsinfoCSV()}
            >
              Confirm
            </Button>
            <Button
...

        {
          <div>
            ...
            >
              API documentation for coursedetailsinfo
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default CoursedetailsinfoTable;
